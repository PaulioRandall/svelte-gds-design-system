<script>
	...
	if (large && small) {
		throw new Error(
			'A text restyle cannot be both leading large and small at the same time'
		)
	}
	...
	if (numOfOverrides > 1) {
		throw new Error(
			'A text restyle cannot have more than one font size override'
		)
	}
</script>

<span
	class:govuk-body-l="{large}"
